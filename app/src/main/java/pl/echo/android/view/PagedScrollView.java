...
public class PagedScrollView extends FrameLayout {
    ...
    private void alignToPage(int pageDelta) {
    	int h = getHeight();
    	int pageHeight = (int) (pageRatio * h);
    	int inset = (h - pageHeight)/2;
    	int sy = getScrollY();
		int currentPage = (sy + h/2 - inset)/pageHeight;
		int maxPage = (getChildAt(0).getHeight() - 2*inset)/pageHeight;
		if(pageDelta > 0 && sy < currentPage * pageHeight) {
			pageDelta = 0;
		} else if(pageDelta < 0 && sy > currentPage * pageHeight) {
			pageDelta = 0;
		}
		int destPage = Math.max(0, Math.min(currentPage + pageDelta, maxPage));
		int startX = getScrollX();
		int destY = destPage * pageHeight;
		smoothScrollTo(startX, destY);
    }
