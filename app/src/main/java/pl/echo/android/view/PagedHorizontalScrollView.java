...
public class PagedHorizontalScrollView extends FrameLayout {
    ...
    public PagedHorizontalScrollView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public PagedHorizontalScrollView(Context context, AttributeSet attrs, int defStyle) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if ((Math.abs(initialVelocity) >
                        ...
                        getChildCount() > 0) {
                	alignToPage(initialVelocity < 0 ? 1 : -1);
                } else {
                	alignToPage(0);
                }
    ...
    private void alignToPage(int pageDelta) {
    	int w = getWidth();
    	int pageWidth = (int) (pageRatio * w);
    	int inset = (w - pageWidth)/2;
    	int sx = getScrollX();
		int currentPage = (sx + w/2 - inset)/pageWidth;
		int maxPage = (getChildAt(0).getWidth() - 2*inset)/pageWidth;
		if(pageDelta > 0 && sx < currentPage * pageWidth) {
			pageDelta = 0;
		} else if(pageDelta < 0 && sx > currentPage * pageWidth) {
			pageDelta = 0;
		}
		int destPage = Math.max(0, Math.min(currentPage + pageDelta, maxPage));
		smoothScrollTo(destPage * pageWidth, getScrollY());
    }
	...
	protected float pageRatio = 0.5f;
